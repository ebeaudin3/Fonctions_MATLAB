 function [xCurrent,Resnorm,FVAL,EXITFLAG,OUTPUT,LAMBDA,JACOB] = lsqcurvefit_e(FUN,xCurrent,XDATA,YDATA,LB,UB,options,varargin)
...
%   $Revision: 1.1.6.10 $  $Date: 2011/07/31 13:15:53 $
...
if nargin == 1
    if isa(FUN,'struct')
        ...
        error(message('optimlib:lsqcurvefit_e:InputArg'));
    end
end

if nargin < 4 && ~problemInput
    error(message('optimlib:lsqcurvefit_e:NotEnoughInputs'));
end
...
if ~strcmp(dataType,'double')
    error(message('optimlib:lsqcurvefit_e:NonDoubleInput'))
end

caller = 'lsqcurvefit_e';
[funfcn_x_xdata,mtxmpy_xdata,flags,sizes,funValCheck,xstart,lb,ub,EXITFLAG, ...
    ...
    lsqnsetup(FUN,xCurrent,LB,UB,options,defaultopt,caller,nargout,length(varargin));
if earlyTermination
...
switch funfcn_x_xdata{1}
    case 'fun'
        try
            ...
            optim_ME = MException('optimlib:lsqcurvefit_e:InvalidFUN', ...
                getString(message('optimlib:lsqcurvefit_e:InvalidFUN')));
    ...
    case 'fun_then_grad'
        ...
        try    
            ...
            optim_ME = MException('optimlib:lsqcurvefit_e:InvalidFUNJac', ...
                getString(message('optimlib:lsqcurvefit_e:InvalidFUNJac')));
            userFcn_ME = addCause(userFcn_ME,optim_ME);
            rethrow(userFcn_ME)
        end
        funfcn{3} = @objective;
        funfcn{4} = @jacobian;
    otherwise
        error(message('optimlib:lsqcurvefit_e:UndefCallType'))
end

if ~isequal(size(initVals.F),size(YDATA))
    error(message('optimlib:lsqcurvefit_e:YdataSizeMismatchFunVal'))
end
